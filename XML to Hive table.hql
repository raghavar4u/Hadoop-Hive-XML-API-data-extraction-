...
wget -S -q -O - http://xxxxxxxxxxxxxxxxxxxx --user=XXXXXX --password=******** > XML_test2.txt
...
drop table XXXXX.XML_test ;

CREATE TABLE XXXX.XML_test
(
xml_test string
);

load data local inpath '/users/XXXX/XML_test2.txt' into table XXX.XML_test; -- Location of the output stored 
...
select xpath_string(xml_test,'Projects/Project/id') from xxxxx.XML_test; -- Header tags will not work in this way since it is with in the projects and global for all the variables 

select xpath_string(xml_test,'Projects/Project/@id') from xxxxx.XML_test; -- if we use "@" it invoke the value from the XML tags 
